...
export async function GET(req: Request) {
  ...
  const endpoints: EndpointDoc[] = [
    // Reports
    { method: 'GET', path: '/api/reports', description: 'List reports. Supports limit, offset, sort, order, pretty.', example: `curl "${origin}/api/reports"` },
    { method: 'POST', path: '/api/reports', description: 'Create a report.', example: `curl -X POST "${origin}/api/reports" -H "content-type: application/json" -d '{"title":"Weekly","body":"notes","week_start":"2025-09-29"}'` },
    { method: 'GET', path: '/api/reports/[id]', description: 'Fetch a single report by id.', example: `curl "${origin}/api/reports/REPLACE_ID"` },
    { method: 'DELETE', path: '/api/reports/[id]', description: 'Delete a report by id.', example: `curl -X DELETE "${origin}/api/reports/REPLACE_ID"` },
